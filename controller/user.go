...
func SearchUsers(c *gin.Context) {
	...
	group := c.Query("group")
	membershipTierId := strings.TrimSpace(c.Query("membership_tier_id"))
	var role *int
	...
	pageInfo := common.GetPageQuery(c)
	sortOptions := model.NewUserSortOptions(c.Query("sort_by"), c.Query("sort_order"))
	users, total, err := model.SearchUsers(keyword, group, membershipTierId, role, status, pageInfo.GetStartIdx(), pageInfo.GetPageSize(), sortOptions)
	if err != nil {
		common.ApiError(c, err)
		return
	}
...
func GenerateAccessToken(c *gin.Context) {
	...
	result := model.DB.Where("access_token = ?", key).First(&model.User{})
...
func UpdateSelf(c *gin.Context) {
	...
	if sidebarModules, sidebarExists := requestData["sidebar_modules"]; sidebarExists {
		...
		if err := model.UpdateUserSetting(userId, user.Setting); err != nil {
...
type UpdateUserSettingRequest struct {
	...
	RecordIpLogUpdatedAt             int64   `json:"record_ip_log_updated_at,omitempty"`
}
