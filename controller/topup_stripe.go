...
func StripeWebhook(c *gin.Context) {
	...
	if !isStripeWebhookEnabled() {
		logger.LogWarn(ctx, fmt.Sprintf("Stripe webhook 被拒绝 reason=webhook_disabled path=%q client_ip=%s", webhookPath, c.ClientIP()))
	...
	if err != nil {
		logger.LogError(ctx, fmt.Sprintf("Stripe webhook 读取请求体失败 path=%q client_ip=%s error=%q", webhookPath, c.ClientIP(), err.Error()))
		c.AbortWithStatus(http.StatusServiceUnavailable)
		return
	}

	signature := c.GetHeader("Stripe-Signature")
	payloadFingerprint := paymentPayloadFingerprint(payload)
	logger.LogInfo(ctx, fmt.Sprintf("Stripe webhook 收到请求 path=%q client_ip=%s signature_present=%t payload_bytes=%d payload_sha256=%s", webhookPath, c.ClientIP(), signature != "", len(payload), payloadFingerprint))
	if signature == "" {
		logger.LogWarn(ctx, fmt.Sprintf("Stripe webhook 缺少签名 path=%q client_ip=%s payload_sha256=%s", webhookPath, c.ClientIP(), payloadFingerprint))
	...
	if err != nil {
		logger.LogWarn(ctx, fmt.Sprintf("Stripe webhook 验签失败 path=%q client_ip=%s payload_sha256=%s error=%q", webhookPath, c.ClientIP(), payloadFingerprint, err.Error()))
		c.AbortWithStatus(http.StatusBadRequest)
		return
	}

	callerIp := c.ClientIP()
	logger.LogInfo(ctx, fmt.Sprintf("Stripe webhook 验签成功 event_id=%s event_type=%s client_ip=%s path=%q payload_sha256=%s", event.ID, string(event.Type), callerIp, webhookPath, payloadFingerprint))
...
func sessionAsyncPaymentFailed(ctx context.Context, event stripe.Event, callerIp string) error {
	...
	return updateStripeOrderTerminalStatus(ctx, referenceId, common.TopUpStatusFailed, "异步支付失败")
}
...
func fulfillOrder(ctx context.Context, event stripe.Event, referenceId string, customerId string, callerIp string) error {
	...
	confirmedAmount := ""
	if amount, err := decimal.NewFromString(event.GetObjectValue("amount_total")); err == nil {
		confirmedAmount = amount.Shift(-2).String()
	}
	...
	if err := model.CompleteSubscriptionOrderWithConfirmation(referenceId, common.GetJsonString(payload), model.PaymentProviderStripe, "", confirmation); err == nil {
	...
	if err != nil {
		if errors.Is(err, model.ErrTopUpNotFound) {
			logger.LogInfo(ctx, fmt.Sprintf("Stripe webhook 忽略非本系统订单 trade_no=%s event_type=%s client_ip=%s", referenceId, string(event.Type), callerIp))
			return nil
		}
...
func sessionExpired(ctx context.Context, event stripe.Event) error {
	...
	return updateStripeOrderTerminalStatus(ctx, referenceId, common.TopUpStatusExpired, "订单过期")
}

func updateStripeOrderTerminalStatus(ctx context.Context, referenceId string, topUpStatus string, eventLabel string) error {
	LockOrder(referenceId)
	defer UnlockOrder(referenceId)
	if err := model.ExpireSubscriptionOrder(referenceId, model.PaymentProviderStripe); err == nil {
		logger.LogInfo(ctx, fmt.Sprintf("Stripe 订阅订单终态处理完成 trade_no=%s event=%s", referenceId, eventLabel))
		return nil
	} else if err != nil && !errors.Is(err, model.ErrSubscriptionOrderNotFound) {
		return err
	}
	...
	if errors.Is(err, model.ErrTopUpNotFound) {
		logger.LogInfo(ctx, fmt.Sprintf("Stripe webhook 忽略非本系统订单 trade_no=%s event=%s", referenceId, eventLabel))
		return nil
	}
	if err != nil {
		return err
	...
	logger.LogInfo(ctx, fmt.Sprintf("Stripe 充值订单终态处理完成 trade_no=%s event=%s status=%s", referenceId, eventLabel, topUpStatus))
	return nil
}
