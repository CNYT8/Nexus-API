...
Nexus-API is maintained as a downstream overlay on top of upstream new-api. For large upstream rewrites, rebuild from the fixed upstream `v1.0.0-rc.12` base and reapply the overlay described here instead of relying on memory, broad file copies, or the remote latest upstream release.
...
- The UI shows the fixed original new-api base `v1.0.0-rc.12`; do not fetch the latest upstream release for this field.
...
## local upstream sync notes (v1.0.0-rc.12)

Synced:

- Default frontend performance fix from upstream `v1.0.0-rc.12`: React Query `refetchOnWindowFocus` is fixed to `false` in `web/default/src/main.tsx`, avoiding heavy automatic refetches when returning focus to log/data-heavy pages.

## Sync process

1. Record current Nexus state and downstream overlay status.
2. Use upstream new-api `v1.0.0-rc.12` as the fixed clean base for large upstream rewrites; do not fetch the remote latest upstream version.
