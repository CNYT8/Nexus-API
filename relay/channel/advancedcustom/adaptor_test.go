...
func TestBuildModelListRequestUsesConfiguredAuth(t *testing.T) {
	tests := []struct {
		name              string
		auth              *dto.AdvancedCustomRouteAuth
		upstreamPath      string
		...
		{name: "default bearer", upstreamPath: "/provider/models", wantAuthorization: "Bearer sk-test"},
		{
			name:         "custom header",
			upstreamPath: "/provider/models",
			auth: &dto.AdvancedCustomRouteAuth{
				Type: dto.AdvancedCustomAuthTypeHeader, Name: "x-api-key", Value: "token {api_key}",
			},
			wantHeader: "token sk-test",
		},
		{
			name:         "query auth",
			upstreamPath: "https://upstream.example/models?existing=1",
			auth: &dto.AdvancedCustomRouteAuth{
				Type: dto.AdvancedCustomAuthTypeQuery, Name: "key", Value: "{api_key}",
			},
			wantQuery: "sk-test",
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			require.NoError(t, err)
			assert.Equal(t, tt.wantAuthorization, headers.Get("Authorization"))
			assert.Equal(t, tt.wantHeader, headers.Get("x-api-key"))
			assert.Equal(t, tt.wantQuery, parsed.Query().Get("key"))
		})
	}
}

func TestBuildModelListRequestDoesNotReuseResolvedRelayRoute(t *testing.T) {
	adaptor := &Adaptor{}
	info := advancedCustomRelayInfo(&dto.AdvancedCustomConfig{Routes: []dto.AdvancedCustomRoute{
		{IncomingPath: "/v1/chat/completions", UpstreamPath: "/chat"},
		{IncomingPath: dto.AdvancedCustomModelListPath, UpstreamPath: "/provider/models"},
	}})
	...
	modelURL, _, err := adaptor.BuildModelListRequest(info)
	require.NoError(t, err)
