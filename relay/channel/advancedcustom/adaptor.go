...
// BuildModelListRequest resolves the explicitly configured /v1/models route
// without mutating adaptor state used by a relay request.
func (a *Adaptor) BuildModelListRequest(info *relaycommon.RelayInfo) (string, http.Header, error) {
	...
	case dto.AdvancedCustomAuthTypeHeader:
		headers.Set(strings.TrimSpace(auth.Name), applyAuthTemplate(auth.Value, info.ApiKey))
	default:
