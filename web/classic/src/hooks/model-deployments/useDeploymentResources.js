  ...
  const fetchHardwareTypes = useCallback(async () => {
    try {
      ...
      if (response.data.success) {
        ...
        showError(i18next.t('获取硬件类型失败: ') + response.data.message);
        setHardwareTotalAvailable(0);
        return [];
      }
    } catch (error) {
      showError(i18next.t('获取硬件类型失败: ') + error.message);
      setHardwareTotalAvailable(0);
      return [];
    } finally {
  ...
  const fetchLocations = useCallback(async (hardwareId, gpuCount = 1) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        showError(i18next.t('获取部署位置失败: ') + response.data.message);
        setLocationsTotalAvailable(0);
        return [];
      }
    } catch (error) {
      showError(i18next.t('获取部署位置失败: ') + error.message);
      setLocationsTotalAvailable(0);
      return [];
    } finally {
    ...
    async (hardwareId, gpuCount = 1) => {
      ...
      try {
        ...
        if (response.data.success) {
          ...
          showError(i18next.t('获取可用资源失败: ') + response.data.message);
          setAvailableReplicas([]);
          return [];
        }
      } catch (error) {
  ...
  const calculatePrice = useCallback(async (params) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        showError(i18next.t('价格计算失败: ') + response.data.message);
        setPriceEstimation(null);
        return null;
      }
    } catch (error) {
  ...
  const checkClusterNameAvailability = useCallback(async (name) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        showError(i18next.t('检查名称可用性失败: ') + response.data.message);
        return false;
      }
    } catch (error) {
