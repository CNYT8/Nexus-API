...
type Log struct {
	...
	CreatedAt         int64  `json:"created_at" gorm:"bigint;index:idx_created_at_id,priority:1;index:idx_created_at_type;index:idx_logs_model_monitor,priority:2"`
	Type              int    `json:"type" gorm:"index:idx_created_at_type;index:idx_logs_model_monitor,priority:1"`
	Content           string `json:"content"`
	Username          string `json:"username" gorm:"index;index:index_username_model_name,priority:2;default:''"`
	TokenName         string `json:"token_name" gorm:"index;default:''"`
	ModelName         string `json:"model_name" gorm:"index;index:index_username_model_name,priority:1;index:idx_logs_model_monitor,priority:3;default:''"`
...
func resolveRecordIpLog(userId int) bool {
	...
	if settingMap.RecordIpLogUpdatedAt >= common.DefaultRecordIpLogUpdatedAt && settingMap.RecordIpLogUpdatedAt > 0 {
...
func RecordErrorLog(c *gin.Context, userId int, channelId int, modelName string, tokenName string, content string, tokenId int, useTimeSeconds int,
	...
	needRecordIp := resolveRecordIpLog(userId)
	log := &Log{
		UserId:           userId,
		Username:         username,
		CreatedAt:        common.GetTimestamp(),
...
func SumUsedQuota(logType int, startTimestamp int64, endTimestamp int64, modelName string, username string, tokenName string, channel int, group string) (stat Stat, err error) {
	quotaCondition := "1 = 1"
	quotaArgs := make([]any, 0, 2)
	if startTimestamp != 0 && endTimestamp != 0 {
		quotaCondition = "created_at >= ? AND created_at <= ?"
		quotaArgs = append(quotaArgs, startTimestamp, endTimestamp)
	} else if startTimestamp != 0 {
		quotaCondition = "created_at >= ?"
		quotaArgs = append(quotaArgs, startTimestamp)
	} else if endTimestamp != 0 {
		quotaCondition = "created_at <= ?"
		quotaArgs = append(quotaArgs, endTimestamp)
	}
	lastMinute := time.Now().Add(-60 * time.Second).Unix()
	selectArgs := append([]any{}, quotaArgs...)
	selectArgs = append(selectArgs, lastMinute, lastMinute)
	tx := LOG_DB.Table("logs").Select(
		"COALESCE(SUM(CASE WHEN "+quotaCondition+" THEN quota ELSE 0 END), 0) quota, "+
	...
	if len(quotaArgs) > 0 {
		rangeArgs := append([]any{}, quotaArgs...)
		rangeArgs = append(rangeArgs, lastMinute)
		tx = tx.Where("("+quotaCondition+") OR created_at >= ?", rangeArgs...)
	}
