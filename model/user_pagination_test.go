...
func TestSearchUsersSortPreservesMembershipFilter(t *testing.T) {
	truncateTables(t)
	insertUsersForPaginationTest(t, 4)
	require.NoError(t, DB.Create(&UserMembership{UserId: 2, TierId: "plus", Source: MembershipSourceManual}).Error)
	require.NoError(t, DB.Create(&UserMembership{UserId: 4, TierId: "plus", Source: MembershipSourceManual}).Error)

	users, total, err := SearchUsers("user", "", "plus", nil, nil, 0, 20, NewUserSortOptions("id", "asc"))
	require.NoError(t, err)
	assert.Equal(t, int64(2), total)
	assert.Equal(t, []int{2, 4}, collectUserIDs(users))
}

func TestNewUserSortOptionsRejectsUnsupportedColumns(t *testing.T) {
	options := NewUserSortOptions("password desc", "asc")
	assert.Equal(t, "id", options.SortBy)
