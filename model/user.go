...
func (options UserSortOptions) Apply(query *gorm.DB) *gorm.DB {
	...
	ordered := query.Order(clause.OrderByColumn{
		Column: clause.Column{Table: "users", Name: columnName},
		Desc:   options.SortOrder != "asc",
	})
	if columnName != "id" {
		ordered = ordered.Order(clause.OrderByColumn{
...
func SearchUsers(keyword string, group string, membershipTierId string, role *int, status *int, startIdx int, num int, sortOptions ...UserSortOptions) ([]*User, int64, error) {
	var users []*User
	var total int64
	var err error
	...
	if membershipTierId != "" {
		query = query.Joins("JOIN user_memberships ON user_memberships.user_id = users.id").
			Where("user_memberships.tier_id = ?", membershipTierId)
	}
	if role != nil {
