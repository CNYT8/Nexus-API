...
var ErrWaffoPancakeWebhookOrderInvalid = errors.New("invalid Waffo Pancake webhook order")
...
func ResolveWaffoPancakeTradeNo(event *WaffoPancakeWebhookEvent) (string, error) {
	if event == nil {
		return "", fmt.Errorf("%w: missing event", ErrWaffoPancakeWebhookOrderInvalid)
	}
	tradeNo := strings.TrimSpace(event.Data.OrderMerchantExternalID)
	if tradeNo == "" {
		return "", fmt.Errorf("%w: missing orderMerchantExternalId", ErrWaffoPancakeWebhookOrderInvalid)
	}
	topUp, err := model.GetTopUpByTradeNoWithError(tradeNo)
	if err != nil {
		return "", fmt.Errorf("resolve Waffo Pancake order: %w", err)
	}
	if topUp.PaymentProvider != model.PaymentProviderWaffoPancake {
		return "", fmt.Errorf("%w: Waffo Pancake tradeNo=%s", model.ErrPaymentMethodMismatch, tradeNo)
	}
	...
	if actualIdentity != expectedIdentity {
		return "", fmt.Errorf(
			"%w: Waffo Pancake buyer identity mismatch tradeNo=%s expected=%q actual=%q",
			model.ErrPaymentOrderMismatch,
			tradeNo,
			expectedIdentity,
			actualIdentity,
		)
...
func ResolveWaffoPancakeSubscriptionTradeNo(event *WaffoPancakeWebhookEvent) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("resolve Waffo Pancake subscription order: %w", err)
	}
	if order.PaymentProvider != model.PaymentProviderWaffoPancake {
		return "", fmt.Errorf("%w: Waffo Pancake subscription tradeNo=%s", model.ErrPaymentMethodMismatch, tradeNo)
	}
	expectedIdentity := WaffoPancakeBuyerIdentityFromUserID(order.UserId)
	actualIdentity := strings.TrimSpace(event.Data.MerchantProvidedBuyerIdentity)
	if actualIdentity != expectedIdentity {
		return "", fmt.Errorf(
			"%w: Waffo Pancake buyer identity mismatch for subscription tradeNo=%s expected=%q actual=%q",
			model.ErrPaymentOrderMismatch,
			tradeNo,
			expectedIdentity,
			actualIdentity,
